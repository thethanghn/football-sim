        ...
        class FootballSimulator {
            ...
            _enterClubhouse() {
                ...
                // Three info cards below the illustration: Kits / City / Stadium.
                this._renderStadiumExtras();
            ...
            // Simple hash of a string → 32-bit unsigned int. Used to derive
            // deterministic-but-varied city / stadium info from the city name.
            _hashStr(s) {
                let h = 5381;
            ...
            // Procedural city stats — population, area, climate, today's weather.
            // Major cities (capitals / large hubs) get higher numbers; smaller
            // cities scale down. Values are deterministic per city name so the
            // user's club always shows the same numbers across sessions.
            _cityInfo(cityName, nation) {
                if (!cityName) return null;
                const h = this._hashStr(cityName);
                // Position in the nation's city list — earlier = bigger.
                const cities = nation?.cities || [];
                const idx    = Math.max(0, cities.findIndex(c => c.name === cityName));
                const size   = cities.length || 1;
                const tier   = idx / Math.max(1, size - 1);   // 0 = biggest, 1 = smallest

                // Population: 9.5M (top) → 90k (bottom). Slight randomness per city.
                const popBase = Math.round(9500 * Math.pow(0.55, tier * 3));  // in thousands
                const popJitter = (h % 1500) - 750;
                const populationK = Math.max(60, popBase + popJitter);

                // Area in km² — capitals 1500–3500, small cities 50–250.
                const areaBase = Math.round(2500 * Math.pow(0.62, tier * 2.4));
                const areaJitter = ((h >> 8) % 400) - 200;
                const area = Math.max(45, areaBase + areaJitter);

                // Climate band from the nation's code (Southeast Asia is mostly
                // tropical / equatorial / monsoonal; the lookup gives a label).
                const climateByCode = {
                    VN: 'Tropical · monsoon',
                    TH: 'Tropical savanna',
                    ID: 'Equatorial · rainforest',
                    MY: 'Equatorial · humid',
                    PH: 'Tropical · typhoon belt',
                    SG: 'Equatorial · humid',
                    LA: 'Tropical · monsoon',
                    KH: 'Tropical · monsoon',
                    MM: 'Tropical · monsoon',
                    BN: 'Equatorial · humid',
                    TL: 'Tropical · two seasons',
                };
                const climate = climateByCode[nation?.code] || 'Tropical';

                // Today's weather — temperature (°C) + condition. Tropical SEA
                // sits roughly 24–34 °C year-round; we shake the value by both
                // the city hash and the current in-game month.
                const dateISO = (typeof GameStorage !== 'undefined') ? GameStorage.loadCurrentDate() : null;
                const month   = dateISO ? new Date(dateISO).getUTCMonth() + 1 : 6;
                const seasonal = (Math.abs(7 - month) <= 2) ? -2 : 0;  // mid-year a touch cooler
                const tempC = 26 + ((h >> 4) % 6) + seasonal;          // 24–32 °C
                const wOpts = ['☀ Sunny', '⛅ Partly cloudy', '🌧 Light rain', '⛈ Thunderstorms', '🌥 Overcast'];
                const condition = wOpts[(h >> 12) % wOpts.length];

                return {
                    population: populationK >= 1000
                        ? (populationK / 1000).toFixed(populationK >= 5000 ? 1 : 2) + ' M'
                        : populationK.toLocaleString() + ' k',
            ...
            // Procedural stadium stats — capacity + ticket prices derived from
            // the club's budget. Bigger clubs get a bigger ground and pricier
            // seats. Returns formatted strings ready for the info card.
            _stadiumInfo(club) {
                if (!club) return null;
                const budget = club.budget || 60;
                // Capacity 8 k (small) → 65 k (mega). Logarithmic-ish scale.
                const cap = Math.round(7000 + (budget / 100) * 35000 + Math.pow(budget / 100, 1.7) * 12000);
                // Ticket bands: standard = ~£12 + budget-scaled premium = ~£60+
                const standard = Math.round(10 + budget * 0.45);
                const premium  = Math.round(40 + budget * 1.8);
                // Year founded — older for capital sides, newer for upstarts.
                const h = this._hashStr(club.clubName || 'X');
                const founded = 1900 + (h % 80);
                return {
                    name: `${club.cityName || club.clubName || 'Home'} Stadium`,
                    capacity: cap.toLocaleString() + ' seats',
            ...
            // Compact shirt SVG used by the Kits card. Body + sleeves + collar,
            // coloured with `color`. label sits underneath.
            _renderKitSVG(color, label) {
                const c = color || '#888';
                return `
                <div class="ch-kit">
                    <svg viewBox="0 0 100 110" xmlns="http://www.w3.org/2000/svg" class="ch-kit-svg">
                        <path d="M30 18 L42 14 L50 18 L58 14 L70 18 L86 32 L78 42 L70 36 L70 96 L30 96 L30 36 L22 42 L14 32 Z"
                              fill="${c}" stroke="#0a0a0a" stroke-width="1.2" stroke-linejoin="round"/>
                        <path d="M42 14 L50 26 L58 14" fill="none" stroke="#0a0a0a" stroke-width="1.2" stroke-linejoin="round"/>
                        <path d="M30 36 L30 50 L24 50" fill="none" stroke="rgba(0,0,0,0.20)" stroke-width="1"/>
                        <path d="M70 36 L70 50 L76 50" fill="none" stroke="rgba(0,0,0,0.20)" stroke-width="1"/>
                    </svg>
                    <span class="ch-kit-label">${label}</span>
                </div>`;
            }

            // Fills the three Stadium-view info cards (Kits / City / Stadium).
            // Called from _enterClubhouse alongside _renderStadiumSVG.
            _renderStadiumExtras() {
                const team = this.playerTeam;
                const mgr  = (typeof GameStorage !== 'undefined') ? GameStorage.loadManager() : null;
                const league = (typeof GameStorage !== 'undefined') ? GameStorage.loadLeague() : null;
                const club   = Array.isArray(league) ? league.find(c => c.isUserClub) : null;
                const nation = (mgr?.nation && typeof SEA_NATIONS !== 'undefined')
                    ? SEA_NATIONS.find(n => n.code === mgr.nation) : null;
                const cityName = mgr?.city || club?.cityName;

                // ── Kits card ───────────────────────────────────────────────
                const home = team?.homeColor || team?.jerseyColor || '#FFD700';
                const away = team?.awayColor || (typeof Team !== 'undefined' ? Team.computeAwayColor(home) : '#FFFFFF');
                const kitsEl = document.getElementById('chKitsBody');
                if (kitsEl) {
                    kitsEl.innerHTML = `
                        ${this._renderKitSVG(home, 'Home')}
                        ${this._renderKitSVG(away, 'Away')}
                    `;
                }

                // ── City card ───────────────────────────────────────────────
                const cityEl = document.getElementById('chCityBody');
                if (cityEl) {
                    const ci = this._cityInfo(cityName, nation);
                    if (!ci) {
                        cityEl.innerHTML = `<div class="ch-stad-empty">No city data.</div>`;
                    } else {
                        cityEl.innerHTML = `
                            <div class="ch-stad-row"><span class="csl-key">Name</span><span class="csl-val">${cityName}</span></div>
                            <div class="ch-stad-row"><span class="csl-key">Population</span><span class="csl-val">${ci.population}</span></div>
                            <div class="ch-stad-row"><span class="csl-key">Area</span><span class="csl-val">${ci.area}</span></div>
                            <div class="ch-stad-row"><span class="csl-key">Climate</span><span class="csl-val">${ci.climate}</span></div>
                            <div class="ch-stad-row"><span class="csl-key">Today</span><span class="csl-val">${ci.weather}</span></div>
                        `;
                    }
                }

                // ── Stadium card ────────────────────────────────────────────
                const stadEl = document.getElementById('chStadBody');
                if (stadEl) {
                    const si = this._stadiumInfo(club || { cityName, clubName: team?.clubName, budget: team?.budget });
                    if (!si) {
                        stadEl.innerHTML = `<div class="ch-stad-empty">No stadium data.</div>`;
                    } else {
                        stadEl.innerHTML = `
                            <div class="ch-stad-row"><span class="csl-key">Ground</span><span class="csl-val">${si.name}</span></div>
                            <div class="ch-stad-row"><span class="csl-key">Capacity</span><span class="csl-val">${si.capacity}</span></div>
                            <div class="ch-stad-row"><span class="csl-key">Founded</span><span class="csl-val">${si.founded}</span></div>
                            <div class="ch-stad-row"><span class="csl-key">Standard</span><span class="csl-val">${si.standardPrice}</span></div>
                            <div class="ch-stad-row"><span class="csl-key">Premium</span><span class="csl-val">${si.premiumPrice}</span></div>
                        `;
                    }
                }
            }
